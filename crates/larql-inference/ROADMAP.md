...
- **Follow-up (C1):** make this KV-cached + engine-selectable. The MoE branch belongs in the engine layer — primary tracking in [larql-kv ROADMAP](../../larql-kv/ROADMAP.md) §"MoE-aware KV engines (C1)". The larql-inference piece is the **`RemoteMoeFfn` `FfnBackend` wrapper** (`{ weights, remote }`) whose `forward_moe_full_layer` runs the `run_moe_layer_cpu` body (dense local + experts remote + combine), so `--moe-shards` rides any `KvEngine` and unifies with the `--ffn` MoE path. Decision 2026-05-28: keep the full-recompute fix as the correctness baseline; this replaces it for perf.
